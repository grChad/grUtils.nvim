...
local function register_autocommands()
   local group_id = vim.api.nvim_create_augroup(autocommand_group, { clear = true })
...
local function get_highlight(line_number, level)
   ...
   local step_index = ((level - 1) % #highlight_steps) + 1
...
function M.get_ribbon()
   return '%#FoldColumnDynamic#' .. "%{v:lua.require('modules.fold').apply_highlight(v:lnum)}" .. '%##'
end
